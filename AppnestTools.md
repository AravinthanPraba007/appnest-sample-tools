...
- **Frontend → backend:** `window.AppnestFunctions.$app.backend({ functionName, functionPayload })`; response `{ statusCode, body }`. No react/react-dom in frontend package.json. → [Appnest-App-Development-Guide.md](appnest-governance/Appnest-App-Development-Guide.md)
