...
2. **Appnest Functions usage:** Confirm all external HTTP use `$fetch.request`; all persistent state use $db; long-running/chained work use $schedule or $next; no axios/fetch; handlers use **`({ payload })`** and return a plain object or **`new ResultData({ body, statusCode })`**; no `@sparrowengg/appnest-app-sdk-utils` in backend package.json. Code that reads `$db` / `$fetch` / `$file` / `$next` / `$schedule` results must use the **documented return property names** from **Backend-Appnest-Functions.md** (no guessed field names). If the app uses $file, $schedule, or $next, confirm the corresponding PRD section (07 file storage, 08 scheduled jobs, 08 function chaining) is filled.
