...
| 4.2 | **Frontend entry** is **app-frontend/src/App.jsx**; backend is called only via **window.AppnestFunctions.$app.backend({ functionName, functionPayload })**. | Yes | ☐ Pass ☐ Fail ☐ Unclear ☐ N/A | 06-technical-architecture; ref. `appnest-governance/Appnest-App-Development-Guide.md`. |
| 4.3 | All external HTTP uses **`$fetch.request`** (no axios/fetch). All persistent state uses **$db**. Long-running/chained work uses **$schedule** or **$next**. Code that reads **`$db` / `$fetch` / `$file` / `$next` / `$schedule`** results uses **documented return property names** from **Backend-Appnest-Functions.md** (no guessed keys). **`$*`** helpers and **`getTraceId`** are taken from **`AppnestFunctions`** at **module** scope only—not passed as arguments between functions. | Yes | ☐ Pass ☐ Fail ☐ Unclear | 06 + 09; ref. **Appnest Functions** (`appnest-governance/appnest-functions/`). |
...
| 4.6 | Handlers use **`({ payload })`** and return **`new ResultData({ body, statusCode })`** or a plain object; no throwing raw errors without structured return. | Yes | ☐ Pass ☐ Fail ☐ Unclear | 09-non-functional; ref. **Appnest Functions** / backend reference. |
