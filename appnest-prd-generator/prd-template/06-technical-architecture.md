...
- **`AppnestFunctions` in code:** In each backend module, use **`const { AppnestFunctions, ResultData } = require('@sparrowengg/appnest-app-sdk-utils');`** then **`const { $db, $fetch, ... } = AppnestFunctions`** at **module** scope. Do **not** pass **`$fetch`**, **`$db`**, or other **`$*`** helpers (or **`getTraceId`**) as function parameters between helpers—see **Helper usage (no DI-style parameters)** in **Backend-Appnest-Functions.md**.
